...
module.exports = async (req, res) => {
    try {
        ...
        let access_token = await generate('access',{ id: user._id, email: user.email, access: 'user', type: 'access_token', expires: Date.now() + 30000*60*1000 });
        let refresh_token = await generate('refresh',{ id: user._id, email: user.email, access: 'user', type: 'refresh_token', expires: Date.now() + 30000*60*1000 });
